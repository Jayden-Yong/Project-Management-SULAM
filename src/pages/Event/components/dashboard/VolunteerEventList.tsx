            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 min-h-[200px]">
                {bookmarkedEvents.map(event => (
                    <div key={event.id} className="bg-white p-5 rounded-2xl border border-slate-100 shadow-sm flex flex-col justify-between hover:shadow-md transition-shadow">
                        <div className="mb-4">
                            <h4 className="font-bold text-slate-900 text-base line-clamp-2">{event.title}</h4>
                            <p className="text-xs text-slate-500 mt-2">📅 {event.date} • 📍 {event.location}</p>
                        </div>
                        <button
                            onClick={() => onViewEvent(event.id)}
                            className="w-full py-2.5 bg-primary-50 text-primary-700 text-sm rounded-xl font-bold hover:bg-primary-100 transition-colors"
                        >
                            View Details
                        </button>
                    </div>
